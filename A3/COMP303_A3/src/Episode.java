...
public class Episode implements Playable {
    ...
    @Override
    public void play() {
        System.out.println("Now playing " + aPodcast.getaName() + " [" + aEpisodeNumber + "]: " + aTitle);
    }

    public boolean equals(Episode pEpisode){
        if (pEpisode == null){
            return false;
        }else{
            return pEpisode.aEpisodeNumber == this.aEpisodeNumber && Objects.equals(pEpisode.aTitle, this.aTitle);
        }
    }

    public String toString(){
        return "Episode"+"["+this.aEpisodeNumber+"]: "+this.aTitle+" from Podcast "+this.aPodcast;
    }
}
